...
{
    //Strategy option to calculate employee with monthly payment Work Status
    internal class MonthPayment : IPayment
    {
        public Payment MakePayment(EmployeeUser user, int month, int year)
        {
            ...
            payment.NormalPay = Math.Round(user.PayRate, 2);
            payment.HolidayPay = Math.Round(payment.HolidayHours * user.PayRate / 22 / 8, 2);
