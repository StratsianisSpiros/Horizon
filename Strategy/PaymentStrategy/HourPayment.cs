...
{
    //Strategy option to calculate employee with hourly payment Work Status
    internal class HourPayment : IPayment
    {
        public Payment MakePayment(EmployeeUser user, int month, int year)
        {
            ...
            payment.NormalPay = Math.Round(payment.WorkHours * user.PayRate, 2);
            payment.HolidayPay = Math.Round(payment.HolidayHours * user.PayRate, 2);
